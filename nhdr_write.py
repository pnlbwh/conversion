...
def main():
    ...
    print('Converting ', args.nifti)

    if args.nifti.endswith('.nii.gz'):
        encoding = 'gzip'
    ...
    if dim == 3 and (args.bval or args.bvec):
...
type: {numpy_to_nrrd_dtype[dtype.name]}\ndimension: {dim}\nspace: right-anterior-superior')
    ...
    if dim==4:
        ...
        if not (args.bval and args.bvec):
            raise ValueError('nifti image is 4D, bval and bvec files are required')

        bvecs = read_bvecs(args.bvec, assumed_norm= False)
        bvals = read_bvals(args.bval)

        print(f'data file: {args.nifti}')
